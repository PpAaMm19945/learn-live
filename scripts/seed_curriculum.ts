...
// Map strand number to strand_id
  ...
  for (const file of files) {
    ...
    for (const t of templates) {
      ...
      if (!seenCapacities.has(capKey)) {
        ...
        // Use task_type as a rough name for the capacity
        sql += `INSERT OR IGNORE INTO Capacities (id, strand_id, band_id, name) VALUES ('${capKey}', '${strandId}', '${bandId}', ${escSQL(t.task_type)});\n`;
      }
    }
  }
  sql += `\n`;
