...
export function handleToolCall(
  ...
  switch (message.tool) {
    ...
    case 'highlight_region': {
      ...
      // Debounced fit — will capture all markers from the same beat
      clearTimeout((handleToolCall as any)._fitTimer);
      (handleToolCall as any)._fitTimer = setTimeout(() => canvas.fitToMarkers(), 300);
      break;
    }
    case 'draw_route': {
