...
export type LogContext = '[UI]' | '[AGENT]' | '[DB]' | '[AUTH]' | '[CORE]' | '[MATRIX]' | '[PROFILE]' | '[SEC]' | '[EVIDENCE]' | '[WITNESS]' | '[EXPLAINER]' | '[NARRATOR]' | '[SYNC]' | '[DEMO]' | '[LOGIN]' | '[ONBOARD]' | '[APP]' | '[LEARNER_STORE]' | '[WS]' | '[AUDIO]' | '[GOLDEN_SCRIPT]';
